...
#     arr = np.loadtxt(os.path.join(os.getcwd(), "PUV0.txt"), delimiter="\t")
#     rows, cols = arr.shape
...
#     p_p = p_arr.reshape((N, M))
#     u_p = u_arr.reshape((N,M))
#     v_p = v_arr.reshape((N, M))
...
U_star=np.zeros((T+1,N+1,M+1),dtype="double")
...
while b:
    ...
    for i in range(T):
        for j in range(1,N):
            for k in range(1,M):
                if k>M*6.5/L_y and j<5*N/L_x:
                    continue
                U_star[i][j]=(Conx/(dx*dx)-U[i][j][k]/dx)*U[i][j+1][k]+(-Conx*2/(dx*dx)-Conx*2/(dy*dy)+U[i][j][k]/dx+V[i][j][k]/dy)*U[i][j][k]+ Conx*(U[i][j-1][k]/(dx*dx)+U[i][j][k-1]/(dy*dy))+(Conx/(dy*dy)-U[i][j][k]/dy)*U[i][j][k+1]
                V_star[i][j]=(Cony/(dx*dx)-U[i][j][k]/dx)*V[i][j+1][k]+(-Cony*2/(dx*dx)-Cony*2/(dy*dy)+U[i][j][k]/dx+V[i][j][k]/dy)*V[i][j][k]+ Cony*(V[i][j-1][k]/(dx*dx)+V[i][j][k-1]/(dy*dy))+(Cony/(dy*dy)-V[i][j][k]/dy)*V[i][j][k+1]
