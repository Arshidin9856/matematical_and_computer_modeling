...
index_cut_x = int(N/10)
index_cut_y = int(M/10)
plt.contourf(X,Y,V[-1], alpha=0.5, cmap="plasma")
plt.title(f"Contour of Velocity V and Velocity direction at iter {t}")
plt.colorbar()
plt.quiver(X, Y, U[-1], V[-1])
# plt.plot(5+U[-1][:,5],Y[:,5],color='black',linewidth=3)
# plt.plot(20+U[-1][:,20],Y[:,20],color='black',linewidth=3)
# plt.plot(40+U[-1][:,40],Y[:,40],color='black',linewidth=3)
# plt.plot(50+U[-1][:,-2],Y[:,-2],color='black',linewidth=3)
